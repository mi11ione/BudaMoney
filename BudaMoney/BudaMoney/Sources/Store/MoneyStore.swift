...
final class MoneyStore: ObservableObject {
    ...
    init() {
        ...
        availableUpgrades = ongoingEvents.flatMap(\.upgrades) + (century?.upgrades ?? [])
    }

    func click() {
        var amount: Double = 1

        for item in appliedUpgrades + ongoingEvents {
            amount = item.affectMoney(amount, currentDay)
        }
    ...
    private func save() {
        ...
        appliedUpgradesIds = appliedUpgrades.map(\.id)
    }
}
